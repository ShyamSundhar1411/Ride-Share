...
def send_notification_on_expiration_to_host(self,ride_id,user_id):
    ...
    content = '''Hello {} hope you are doing well. We are Sorry to inform that your ride from {} to {} which was about to start by {} has been expired automatically by our website norms
    Thank You
    Best Regards
    '''
...
def expirehostedride(self,ride_id,user_id):
    ...
    if ride.status == "OPEN":
        ride.status = "EXPIRED"
        send_notifications_on_expirations_to_host(ride_id,user_id)
        ride.save()
    else:
        ride.status = "EXPIRED"
        ride.save()
...
def send_notification_on_cancellation(self,ride_id,cancelled_id):
    ...
    message.send()
@shared_task(bind  = True)
def send_notification_on_expiration_to_pools(self,ride_id):
    ride = RideHost.objects.get(pk = ride_id)
    ...
    subject = 'Test'
    content = 'Hi'
    message = EmailMessage(subject, content, me, email_l)
    message.send()
